...
def load_data(file_path):
    # Read the CSV file with Timestamp column as string
...
if df is not None:
    ...
    if 'Timestamp' in df.columns:
        # Convert scientific notation to datetime format
        df['Timestamp'] = pd.to_datetime(df['Timestamp'].apply(lambda x: pd.Timestamp(float(x)).strftime('%Y-%m-%d %H:%M:%S')), errors='coerce')
